...
import com.ocean.core.commons.annotation.Dict;
...
public class DictAspect {
    ...
    // 定义切点Pointcut
    @Pointcut("execution(public * com.ocean.core.*.*Controller.*(..))")
    ...
    private void parseDictText(Object result) {
        //判断返回结果是否属于 ResultMessage
        if (result instanceof ResultMessage) {
            ResultMessage resultMessage = (ResultMessage) result;
            //判断是否属于成功的
            if (resultMessage.isSuccess()) {
                //获取返回结果
                if (resultMessage.getResult() instanceof IPage) {
                    IPage resultPage = (IPage) resultMessage.getResult();
